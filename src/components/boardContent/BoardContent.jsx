...
const BoardContent = () => {
    ...
    useEffect(() => {
        const fetchBoardAndColumns = async () => {
            try {
                const boardResponse = await axios.get(`https://localhost:7125/getboard/${boardId}`);
                const board = boardResponse.data;
                const columnsOrderFromBoard = board.columnsOrder || [];

                const columnsResponse = await axios.get(`https://localhost:7125/ByBoard/${boardId}`);
                const fetchedColumns = columnsResponse.data;

                const sortedColumns = fetchedColumns.sort((a, b) => 
                    columnsOrderFromBoard.indexOf(a.id.toString()) - columnsOrderFromBoard.indexOf(b.id.toString())
                );

                setColumns(sortedColumns.map(column => ({ ...column, cards: column.cards || [] }))); // Ensure cards are always an array
                setColumnsOrder(columnsOrderFromBoard);
            } catch (error) {
                console.error('Failed to fetch board and columns:', error);
                Navigate('/');
            }
        };

        fetchBoardAndColumns();
    }, [boardId]); // Only depend on boardId to prevent infinite loop
